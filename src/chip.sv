...
(
    ...
    key_scheduler key_scheduler_inst (.clk, .reset, .go, .key_in, .keys_out, .aes_go);
    aes aes_inst (.clk, .reset, .go(aes_go), .ciphertext(ciphertext_in), .keys(keys_out), .plaintext(plaintext_out), .ready(io_out[11]));
    //aes aes_inst(.clk(clk), .ready(io_out[11]), .reset(reset), .go(aes_go), .plaintext(plaintext_out), .key(key_in), .ciphertext(ciphertext_in));
