...
In order to test post tapeout, connect the 12 io_in pins to a simple breadboard circuit. The 8 info pins could be connected to 8 simple switch circuits to drive the pins. io_in[11:8] should be connected to buttons. In order to check if the resulting plaintext is correct, connect each of the 8 output pins to 8 LEDs, then connect the ready output pin to another LED. Using the mini_aes.py script, generate an input ciphertext, an input key, and decrypted plaintext and verify that the outputs match the ones generated by the script. 

To test the RTL implementations, generate a ciphertext, key, and decrypted plaintext using the same Python script, then plug these values into the system verilog test bench called aes_tb.sv under the correct variable names. 
